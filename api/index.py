...
VERSION="1.4.5-debug"
DOE_BASE="https://do-api-web-search.doe.sp.gov.br"; SEARCH_URL=f"{DOE_BASE}/v2/advanced-search/publications"; JOURNALS_URL=f"{DOE_BASE}/v2/journals"
PDF_BASE="https://do-api-publication-pdf.doe.sp.gov.br"; WEB_BASE="https://doe.sp.gov.br"; TIMEOUT=float(os.getenv("DOESP_TIMEOUT_SECONDS","25")); PAGE_SIZE=100; MAX_PAGES=50
...
async def profile(fd,td):
 ...
 matches.sort(key=lambda x:str(x.get("date") or ""));return matches,pages,tr,weak
...
async def frontend_probe(url,text):
 scripts=uniq([urljoin(url,s) for s in re.findall(r'<script[^>]+src=["\']([^"\']+)',text,re.I)])[:30]
 routes=[];apis=[];scanned=[]
 for src in scripts:
  try:r=await get(src,accept="text/javascript,*/*")
  except Exception:continue
  if r.status_code>=400:continue
  body=r.text;scanned.append({"url":src,"size":len(body)})
  for m in re.findall(r'https?://[^"\'`\\\s]+',body):
   if "do-api" in m or "doe.sp.gov.br" in m:apis.append(m[:300])
  for pat in (r'[^"\'`]{0,120}/v[0-9]+/[^"\'`]{0,180}',r'[^"\'`]{0,120}(?:edition|journal|sumario|section|publication)[^"\'`]{0,180}'):
   for m in re.findall(pat,body,re.I):
    if "http" in m or "/v" in m:routes.append(m[:300])
 return {"scripts":scripts,"scanned":scanned,"apiStrings":uniq(apis)[:60],"routeHints":uniq(routes)[:100]}
async def candidate_from_publication(item,deep=False):
 slug=item.get("slug")
 if not slug:return {"status":None,"editionIds":[]}
 url=f"{WEB_BASE}/{str(slug).lstrip('/')}";r=await get(url,accept="text/html,*/*");text=r.text;ed=uniq([m.group(1) for m in EDITION_URL_RE.finditer(text)])
 z={"url":url,"status":r.status_code,"finalUrl":str(r.url),"editionIds":ed,"htmlSize":len(text)}
 if deep:z["frontend"]=await frontend_probe(url,text)
 return z
async def locate(item):
 d=await candidate_from_publication(item);tested=[]
...
async def debug_publication(slug:str=Query(...)):return await candidate_from_publication({"slug":slug},deep=True)
@app.get("/api/search")
...
@app.get("/api/context")
async def context(slug:str=Query(...)):return {"source":"DOE-SP official publication page",**(await candidate_from_publication({"slug":slug},deep=False)),"contextComplete":False}
