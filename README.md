...
O processamento de PDFs é serializado por instância para evitar sobreposição de edições grandes. Transferências interrompidas e downloads inválidos são repetidos uma vez; edições de até 64 MB são aceitas por padrão. Esses limites podem ser ajustados por `DOESP_PDF_DOWNLOAD_ATTEMPTS` e `DOESP_MAX_PDF_BYTES`.
